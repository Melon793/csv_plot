...
> - **光标功能整合**: 将 [cursor_mode_modification_plan.md](file:///Users/demo/Documents/python_repo/csv_plot/cursor_mode_modification_plan.md) 的 "off" 模式功能整合入 CursorManager  
...
### 1.1 项目概况
- **主文件**: [csv_plot_pyqt6.py](file:///Users/demo/Documents/python_repo/csv_plot/csv_plot_pyqt6.py) - 共 6691 行
- **主要问题类**: 
...
代码中存在 **53 处** `self.window()` 调用、**19 处** ViewBox 信号回调中的 `pw.window()` 调用，以及 **53 处** `main_window.xxx` 直接属性访问，散布在几乎所有功能模块中。项目已有 [PlotContext](file:///Users/demo/Documents/python_repo/csv_plot/src/app/plot_context.py) 解耦层，但当前几乎未使用（仅在 `create_subplots_matrix` 中注入，`CustomViewBox` 中少量使用）。
...
- **v3.0 变更**: 整合 [cursor_mode_modification_plan.md](file:///Users/demo/Documents/python_repo/csv_plot/cursor_mode_modification_plan.md) 的 "off" 模式功能，包括 `last_valid_cursor_mode` 状态管理
...
重构后，[csv_plot_pyqt6.py](file:///Users/demo/Documents/python_repo/csv_plot/csv_plot_pyqt6.py) 将只包含：
1. 导入语句
...
现有的 [cursor_manager.py](file:///Users/demo/Documents/python_repo/csv_plot/src/ui/cursor_manager.py) 是一个 Mixin 类，文档中明确写了 `class MyPlotWidget(CursorManager, pg.GraphicsLayoutWidget)` 的用法。这正是之前导致 MRO 问题的根源。当前 `DraggableGraphicsLayoutWidget` 已不使用此 Mixin，但文件仍存在。
...
当前 [PlotServices](file:///Users/demo/Documents/python_repo/csv_plot/src/app/plot_context.py#L14-L47) 协议仅覆盖了部分接口。需要扩展以覆盖所有 `self.window()` 和 `pw.window()` 访问：
