...
**前置测量（硬性步骤，非可选）**：先在 [plot_widget.py](file:///Users/demo/CSV_Plot_PySide/src/ui/widgets/plot_widget.py) 的 `mouse_moved` 入口加 `time.perf_counter()` 计时（沿用 §2.4 思路），在目标 Windows 机器 + 目标数据规模下测出单次 `mouse_moved` 的 P95 耗时。根据测量结果决定目标 rateLimit：
