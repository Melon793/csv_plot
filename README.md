...
- **右键菜单**: 缩放模式、跳转到数据范围、固定游标、调整图表高度等快捷操作
...
2. **浏览数据**
   - 在左侧变量列表中查看所有变量
   - 观察颜色指示了解数据质量
   - 双击变量名查看详细数据表格(也可以通过右键，添加到变量数值表)

3. **绘制图表**
   - 从变量列表拖拽变量到绘图区域 (按住shift可以拖入多个变量)
   - 单个绘图区有单曲线模式、多曲线模式（从单曲线模式变为多曲线模式，可以通过变量编辑器添加新曲线转换）
   - 使用鼠标进行缩放和平移操作
   - 右键菜单提供更多选项（比如高度调整）
...
### 多曲线模式
- **批量添加**: 支持同时拖入多个变量(按住shift)，自动激活多曲线模式，或者在变量编辑器中添加变量（总数>=2）
- **颜色管理**: 自动为每条曲线分配不同颜色
