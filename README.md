...
- **精确控制**:
  ...
  - 双击中键清除单个图表
  - Ctrl/Shift + 滚轮缩放特定轴
- **绘图区右键菜单**: 跳转至数据表、按 X 范围调节 Y 轴、游标模式（单自由 / 单固定 / 双固定 / 关闭）、显示或隐藏游标数值、复制变量名、绘图变量编辑器、调整高度、清除绘图；菜单底部的 pyqtgraph 自带项（View All / X axis / Y axis / Plot Options）保持英文
...
### 智能数据表格
- **快速访问**: 双击变量名或点击绘图区右键菜单的「跳转至数据表」打开表格
- **冻结列功能**: 支持列冻结，方便对比查看
