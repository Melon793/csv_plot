...
class MarkRegionManager:
    ...
    def add_mark_region(self, min_x: float, max_x: float):
        ...
        if self.pw.plot_context:
            self.mark_region.sigRegionChanged.connect(self.pw.plot_context.sync_mark_regions)
    ...
    def _get_mark_stats_multi_curve(self, min_x: float, max_x: float) -> list | None:
        ...
        for var_name, ci in self.pw.curves.items():
            ...
            if ci.x_data is not None and ci.y_data is not None:
                ...
                x_data = self.pw.offset + self.pw.factor * np.arange(1, len(ci.y_data) + 1, dtype=np.float32)
                y_data = ci.y_data
            else:
                curve = ci.curve
            ...
            stats_list.append(MarkStats(x1, x2, y1, y2, dx, dy, slope, label, y_avg, y_max, y_min))
    ...
    def _get_mark_stats_single_curve(self, min_x: float, max_x: float) -> list | None:
        ...
        if hasattr(self.pw, 'original_index_x') and hasattr(self.pw, 'original_y') and self.pw.original_index_x is not None:
        ...
        return [MarkStats(x1, x2, y1, y2, dx, dy, slope, self.pw.label_left.text(), y_avg, y_max, y_min)]
