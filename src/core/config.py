...
# 状态栏上翻抽屉（P0-2）：宽度上限。实测（cocoa 真机，默认字体）600 px 下值列
# 净宽 441 px，约等于 63 个拉丁字符 —— 够放常见工程路径，更长的绝对路径按尾部
# 省略（开头目录保留），完整值走行尾「复制」与 tooltip。再窄则路径中段都读不到。
# 上限只吃宽度不吃高度（值列单行、路径行靠视口滚动），所以放宽不动"矮窗口不弹"
# 那一档；代价是这扇抽屉是盖在绘图区上方的 Popup，宽度就是遮挡宽度
STATUS_DRAWER_WIDTH = 600
# x 轴抽屉的专用宽度上限（文件抽屉沿用上面的 600）：这扇抽屉的内容比文件抽屉
# 窄得多，共用 600 会在右侧白留一大块空。440 是扫出来的 —— 最紧的一行是页脚
