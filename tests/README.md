...
| component（offscreen 控件级） | 38 | ~0.7s | 稳定运行 |
| e2e（主窗口冒烟） | 0 | - | 规划中（见第 8 节） |
| **全量** | **180** | **~3.3s（含 cov）** | `180 passed` |
...
- `src/data` 层 21%~81%（短板：`mdf_lazy_loader` 21%、`excel_loader` 53%）；
- `src/ui` 层多数模块为 0%（e2e 未启动，见第 8 节短板表）。
...
| `uv run pytest -m "unit or component"` | 提交前自检 | 180 passed |
| `uv run pytest` | CI 全量 | 180 passed |
...
| 脚本 | 建议目标 | 断言要点 |
|---|---|---|
| `tmp/verify_freeze_restore_e2e.py` | `component/test_y_freeze.py` | setXRange 后 `autoRange[1]` 保持 True；冻结期 Y 不重算；恢复后 Y 重算正确（7 项断言） |
| `tmp/verify_yrange_jitter_claims.py` | `component/test_y_freeze.py` | 可见段 Y 范围数值断言（约 38.71~61.29） |
| `tmp/verify_ownership_inversion.py` | `component/test_xlink_sync.py` | XLink 级联下交互所有权归属 |
| `tmp/verify_reload_y_visibility.py` | `component/test_plot_widget.py` | reload 后 Y 轴可见性 |
| `tmp/verify_incremental_marks.py` | `component/test_mark_region.py` | 增量区间标记 |
| `tmp/verify_box_zoom_prod_aligned.py` | `component/test_plot_widget.py` | 框选缩放与生产行为一致 |
| `tmp/test_template_conflict_choice.py` | `e2e/test_template_roundtrip.py` | 模板冲突选择路径 |

`diagnose_*` / `trace_*` / `watch_*` / `bisect_*` 类一次性诊断脚本不转正，留在 tmp/ 作历史参考。

## 8. 拓展路线图

### 覆盖率短板（`src/ui` 层 0% 模块，e2e 未启动所致）

| 模块 | 语句数 | 覆盖率 | 破冰途径 |
|---|---|---|---|
| `src/ui/file_loader_manager.py` | 812 | 0% | e2e: load_and_plot |
| `src/ui/main_window.py` | 497 | 0% | e2e: app_boot |
| `src/ui/layout_manager.py` | 588 | 0% | e2e: load_and_plot |
| `src/ui/variable_list.py` | 354 | 0% | e2e: load_and_plot |
| `src/ui/cursor_sync_manager.py` | 467 | 0% | component: 游标用例 |
| `src/ui/splash_screen.py` | 141 | 0% | e2e: app_boot |

### Phase 3：e2e 从 0 到 1

- `test_app_boot.py`：启动 → 主窗口构建 → 正常退出；
- `test_load_and_plot.py`：加载 CSV → 拖拽绘图 → 缩放 → 游标；
