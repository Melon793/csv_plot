...
技术栈：**pytest + pytest-qt + pytest-cov + pytest-xdist + pytest-timeout**，`QT_QPA_PLATFORM=offscreen` 无头运行，三层金字塔组织（unit / component / e2e）+ 按需手跑的 perf 层（默认集已排除）。

| 层 | 用例数 | 实测耗时（串行） | 状态 |
|---|---|---|---|
| unit（纯逻辑，无 Qt） | 755 | ~6.2s | 稳定运行 |
| component（offscreen 控件级） | 474 | ~15.1s | 稳定运行 |
| e2e（主窗口冒烟） | 86 | ~14.1s | 稳定运行 |
| **全量** | **1315**（另有 4 例 `perf` 默认 deselected） | **~37.3s**（并行 `-n 4 --dist loadfile` **12.9s**；带 `--cov` 约 42s） | `1315 passed` |

> 上表数字为 **2026-09-23 在 macOS arm64 / offscreen 实测**（`--durations=20` 口径，
> 分支 `dev/lazy-parquet-loading`）。unit 层对机器负载敏感，同一集合两次实测 6.2s / 8.1s。
> 相比 2026-09-22 基线（1126 例 / 25.7s）新增 189 例，几乎全部来自 CSV→parquet 惰性链
> （P0–P7）：unit +167（`loader_caps` / `probe_csv_schema` / `column_cache` / `temp_cache_dir` /
> `parquet_converter` / `parquet_lazy_loader` / `parquet_equivalence` / `var_info_parquet`）、
> component +18（`lazy_plot_flow` / `enum_column_flow`）、e2e +4（`lazy_load_smoke`）。
> unit 层 2.8s → 6.2s 里约 2.3s 就是这批新用例的真实转换与读盘 I/O（单例最慢 0.12s，
> 没有慢例，是数量与 I/O 的自然成本，不必再优化）。
> 再往前：2026-09-22 的等待改造把 1127 例 / 73.5s 压到 1126 例 / 25.7s（component
> 41.6s → 11.5s，只改等待不删断言，见 §5.3 与陷阱 #12）。
> 更早的 214 例 / ~4.2s 是 Phase 1 快照，早已过期——改数字时请重跑，别顺手上调。

覆盖率现状（`--cov=src`）：

- 总体 **73%**（16691 语句 / 4538 未覆盖）；
- 惰性链新模块：`parquet_lazy_loader` 98%、`parquet_converter` 97%、`var_info` 93%、
  `temp_cache_dir` 86%、`_column_cache` / `loader_caps` 100%；
- 老模块：`main_window` 86%、`excel_loader` 89%、`mdf_lazy_loader` 84%、
  `mark_region_manager` 77%、`font_cache` 67%、`plot_widget` 64%、`layout_manager` 60%、
  `file_loader_manager` 58%（49%→58%）、`cursor_manager` 55%、`variable_list` 45%、
  `curve_strategy` 47%；
- 满分：`template_models` / `plot_config` / `utils.paths` / `theme` / `gc_guard` /
  `sheet_selector` 均 100%；
- 仍是短板：`splash_screen` **15%**（142 语句里 120 未中，启动画面无冒烟用例）、
  `loader_factory` **18%**（49 语句里 40 未中——分派点只被 e2e 间接穿过，开关/阈值/回退
  三条分支缺 unit 级直测）、`time_correction` **21%**（39 语句里 31 未中）、
  `variable_actions` **11%**、`dialogs/axis` **12%**。
...
| 命令 | 用途 | 实测结果 |
|---|---|---|
| `uv run pytest -m unit` | 日常开发高频回归（秒级） | 755 passed / ~6.2s |
| `uv run pytest -m "unit or component"` | 提交前自检 | 1229 passed / ~23.3s |
| `uv run pytest` | CI 全量 | 1315 passed / ~37.3s |
| `uv run pytest -m e2e` | e2e 冒烟（含真实主窗口） | 86 passed / ~14.1s |
| `uv run pytest --cov=src --cov-report=term-missing` | 覆盖率（含未覆盖行号） | 总体 73% |
| `uv run pytest -m component -k legend` | 精准过滤（marker + 关键字组合） | 46 passed / ~1.0s |
| `uv run pytest -m perf` | 惰性链访问成本基准（~13MB 合成夹具，默认集已排除） | 4 passed / ~1.7s |
| `uv run pytest -m "unit or component" -n 4 --dist loadfile` | 提交前自检（并行） | 1229 passed / ~9.5s |
| `uv run pytest -n 4 --dist loadfile` | CI 全量（并行） | 1315 passed / ~12.9s |
...
**并行口径**（2026-09-23 实测：`unit or component` 串行 23.3s → `-n 4 --dist loadfile` 9.5s；
全量串行 37.3s → 12.9s）。选型结论：

- 用 `--dist loadfile`：同文件的用例落在同一 worker，避免"同文件内的用例互相踩环境"
  （`-m unit -n auto` 实测 **无收益**——unit 只有数秒，worker 启动与 import 开销吃掉全部节省，
  日常单跑 unit 直接用串行）；
- e2e 并行实测稳定（1315 passed × 多次），未发现 README 陷阱 #3 类的顺序依赖——因为
...
├── fixtures/
│   ├── data_factory.py          # 合成数据工厂（make_timeseries / write_csv / write_field_like_wide_csv
│   │                            #   / write_xlsx / write_mdf）
│   └── waits.py                 # 等待工具：wait_until / settle / flush_deferred_deletes / pump
├── unit/                        # unit 层：无 Qt 依赖的纯逻辑测试（29 个用例文件）
│   ├── core/                    #   config / plot_config / settings / storage /
│   │                            #   template_* / auto_save
│   ├── test_loader_caps.py      #   is_lazy_loader() 能力谓词（含 MDF/parquet 双入口）
│   ├── data/                    #   loader / metadata / excel / var_info / parquet 链 …
│   │   ├── conftest.py          #   var_info 共用的合成 loader 夹具（mdf4/mdf3/attr4/attr3/csv）
│   │   ├── test_probe_csv_schema.py       # CSV 头部+样本探测（转换器取 schema 的唯一入口）
│   │   ├── test_parquet_converter.py      # CSV/Excel → parquet 转换与 S1–S5 语义
│   │   ├── test_parquet_lazy_loader.py    # 逐列读取 / dtype 还原 / close 线程安全
│   │   ├── test_parquet_equivalence.py    # 与内存 loader 的逐位等价性 + 有意分歧清单
│   │   ├── test_var_info_parquet.py       # parquet 侧变量信息快照
│   │   ├── test_column_cache.py           # 条数 + 字节双预算 LRU（polars estimated_size）
│   │   ├── test_temp_cache_dir.py         # 创建 / 清扫三重保险 / 多级回退 / 心跳
...
│   ├── test_lazy_plot_flow.py   #   惰性 loader 的绘图/取数链路
│   ├── test_enum_column_flow.py #   枚举列在内存/parquet 两路的显示口径（含有意分歧）
│   └── test_*.py                #   legend / viewbox / 表格 / 游标 / 抽屉 / 生命周期
├── e2e/                         # e2e 层：真实 MainWindow 冒烟（10 个用例文件）
│   ├── conftest.py              #   dialog_stubs / main_window / loaded_window 夹具 + 清理竞态过滤
│   ├── test_lazy_load_smoke.py  #   大 CSV 真走转换 → 绘图 → 重载 → 曲线存活
│   └── test_*.py                #   启动 / 加载绘图 / 状态栏 / 抽屉 / 窗口几何
└── perf/                        # perf 层：默认集排除，`uv run pytest -m perf` 手动跑
    └── test_lazy_access_cost.py #   惰性链量级护栏（转换吞吐 / 12 列冷读 / 命中成本 / 枚举还原）
```
...
`write_field_like_wide_csv`（贴近现场宽表形态：表头行尾多一个分隔符、列名内嵌私有区字符、
`**` 缺测标记，三项均开关控制——列名归一与 null 语义的回归都从这里取夹具）。
...
| 13 | 惰性转换用例**看着绿了其实没走转换** | 路由有两道闸门叠加：`FILE_SIZE_LIMIT_BACKGROUND_LOADING`（2MB，决定同步/异步）与 `lazy_convert/min_mb`（默认 50MB），而同步入口 `allow_lazy_convert=False` 恒不转换（D16：转换是 CPU 重活，落 GUI 线程就是整窗冻结） | 测路由必须同时打两个桩（`test_lazy_load_smoke.py::force_async_lazy`），并断言 `is_lazy_loader(mw.loader)`；只打 `min_mb=0` 时 KB 级文件仍走同步内存路——同文件里有一条反向用例专门钉住这点 |
...
| `src/ui/variable_actions.py` | 56 | **11%** | 列表右键与变量信息窗共用出口，两条路径各测一次即可 |
| `src/data/loader_factory.py` | 49 | **18%** | 只有 e2e 间接穿过：补 unit 级开关/阈值/回退三分支（含 `_lazy_fallback_reason` 挂载） |
| `src/ui/dialogs/time_correction.py` | 39 | **21%** | 只有抽屉/游标用例的间接覆盖：补 `test_time_correction.py`（偏移解析与越界分支） |
| `src/ui/cursor_sync_manager.py` | 462 | 48% | ✅ 已破冰（`cursor_x_domain` 系列）；深化：链接/解链与恢复路径 |
| `src/ui/variable_list.py` | 331 | 45% | ✅ 已破冰；深化：搜索/多选/拖拽出口 |
| `src/ui/layout_manager.py` | 633 | 60% | ✅ 已破冰；深化：多子图布局/拖拽重排 |
| `src/ui/file_loader_manager.py` | 871 | 58% | ✅ 已破冰（lazy 链 49%→58%）；深化：reload 全流程、错误路径 |
| `src/ui/main_window.py` | 780 | 86% | ✅ 已破冰；深化：模板菜单/快捷键全集 |

### Phase 3：e2e 从 0 到 1（已完成 ✅ / 剩余项）

- ✅ `test_app_boot.py`：启动 → 主窗口构建 → 初始状态 → 空重载防护；
- ✅ `test_load_and_plot.py`：加载 CSV → 变量列表 → 拖拽绘图 → Ctrl+R/Ctrl+Y 快捷键；
- ✅ `test_lazy_load_smoke.py`：路由/开关/回退三分支（阈值打桩口径，见陷阱 #13）；
...
- `time_correction`（21%）与 `curve_strategy`（47%）、`font_cache`（67%）：src 层遗留空白，
  纯逻辑单元测试即可覆盖；
- `mdf_lazy_loader`（84%）：剩余集中在 asammdf 通道枚举与文件尾解析分支，用合成小型 MDF 补；
  P8 若把 MDF 缓存迁到 `ColumnCache`（实例级 256MB 预算），这批用例是回归底座；
- `excel_loader`（89%）：calamine 快路径已覆盖，缺的是 openpyxl 回退与多 sheet 选择分支；
  Excel 是否开惰性闸（实测转换更慢、峰值更高）待作者决策，未决前无对应用例；
...
- ✅ perf 层落地：`tests/perf/test_lazy_access_cost.py` 4 例钉住惰性链量级（转换吞吐 / 12 列冷读 /
  命中成本 / 枚举还原），扩展方向是降采样与滚轮节流基线；
- GitHub Actions 三平台矩阵（macOS / Windows / Linux，offscreen 模式无需 xvfb）；
- 覆盖率基线护栏：`--cov-fail-under=65` 防回退（2026-09-23 实测总体 73%，留 8 点余量；
  原先计划的 30 早已低于真实值，起不到防回退作用）；
- Windows 侧未验证盲区：UNC 网盘路径与打包产物的 polars 收集只能在办公电脑实测，
  本地全绿不代表该分支已验收。
